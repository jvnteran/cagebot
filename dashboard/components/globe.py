...
Rendered as a self-contained HTML document inside an iframe via st.iframe().
"""
...
def render_globe(points: list[dict], height: int = 520):
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    """
    # st.iframe auto-detects a raw HTML string (no scheme, not a real path) and
    # embeds it as srcdoc. Note it hardcodes the iframe's scrolling=True, where
    # the components.html call this replaced passed scrolling=False — what keeps
    # a scrollbar off the globe is this document's own `body{overflow:hidden}`,
    # so keep that rule if the markup below is ever reworked.
