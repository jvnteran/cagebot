...
SIDEBAR_STATUS = (
    ...
    "<span style='color:#c8c8cf;text-transform:uppercase;letter-spacing:0.1em;'>picks</span>"
    "<span style='color:#c8c8cf;'>pre-event</span></div>"
    "<div style='display:flex;justify-content:space-between;padding:4px 0;"
    "border-bottom:1px dashed #1c1c20;'>"
    "<span style='color:#c8c8cf;text-transform:uppercase;letter-spacing:0.1em;'>access</span>"
    "<span style='color:#c8c8cf;'>read-only</span></div>"
    "<div style='display:flex;justify-content:space-between;padding:4px 0;'>"
...
def configure_page(page_title: str = None, page_icon: str = None,
                   initial_sidebar_state: str = "auto"):
    """Set page config. Must be the FIRST Streamlit call in a page script.

    Every page needs this, not just Overview: Streamlit applies set_page_config
    per script run, and each page under pages/ is its own script. A page that
    omits it falls back to the centered layout, which caps the content column at
    736px while a page that sets layout="wide" gets the full width.
    """
    st.set_page_config(
...
def inject_styles():
    """Call once at the top of each page, after configure_page()."""
