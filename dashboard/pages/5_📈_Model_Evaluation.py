...
from components.styles import configure_page, inject_styles, eyebrow
...
st.markdown(
    ...
    "If the bars trend upward, the model's confidence is meaningful — not noise. "
    "Bar colour is the calibration verdict: how the accuracy a bucket delivered "
    "compares with the confidence it claimed.</p>",
    unsafe_allow_html=True,
)

# Colour answers "was this bucket's stated confidence honest?", so it keys off the
# gap between delivered accuracy and the bucket's own mean predicted probability —
# not off accuracy alone. A 62% bucket hitting 61% is calibrated; a 67% bucket
# hitting 59% is overconfident even though the raw accuracy is similar.
CAL_UNDER = "#3987e5"   # blue       — underconfident: wins far more than it claims
CAL_HONEST = "#22c55e"  # green      — accuracy clears the confidence claimed
CAL_BARELY = "#15803d"  # deep green — lands on its confidence with no margin
CAL_OVER = "#dc2626"    # red        — overconfident: performance trails confidence

# Palette validated for this #060606 surface: worst-pair CVD separation ΔE 8.6
# (deuteranopia), normal-vision ΔE 20.3. Red vs green is the weakest pair under
# colour blindness, so each bar also carries its signed gap as text and the key
# below repeats the bands — colour never carries the verdict alone.
CAL_BANDS = [
    (8.0, CAL_UNDER, "underconfident"),
    (2.0, CAL_HONEST, "confidence honest"),
    (-3.0, CAL_BARELY, "barely clears confidence"),
]


def calibration_verdict(gap_pp: float) -> tuple[str, str]:
    """Map a bucket's accuracy-minus-confidence gap (pp) to its colour + label."""
    for floor, colour, label in CAL_BANDS:
        if gap_pp >= floor:
            return colour, label
    return CAL_OVER, "overconfident"


cal_df["gap_pp"] = cal_df["actual_win_rate"] * 100 - cal_df["avg_predicted"]
_verdicts = [calibration_verdict(g) for g in cal_df["gap_pp"]]
bar_colours = [c for c, _ in _verdicts]
bar_labels = [v for _, v in _verdicts]

st.markdown(
    "<div style='display:flex;flex-wrap:wrap;gap:16px;font-size:11px;color:#c8c8cf;"
    "font-family:JetBrains Mono;margin:0 0 10px;'>"
    + "".join(
        "<span style='display:inline-flex;align-items:center;gap:6px;'>"
        f"<span style='width:10px;height:10px;border-radius:2px;background:{c};'></span>"
        f"{label}</span>"
        for c, label in [
            (CAL_UNDER, "Underconfident (≥ +8pp)"),
            (CAL_HONEST, "Honest (+2 to +8)"),
            (CAL_BARELY, "Barely clears (−3 to +2)"),
            (CAL_OVER, "Overconfident (< −3pp)"),
        ]
    )
    + "</div>",
    unsafe_allow_html=True,
...
fig_conf.add_trace(go.Bar(
    ...
    marker=dict(
        color=bar_colours,
        cornerradius=4,
        line=dict(width=2, color="#060606"),
    ),
    # Accuracy + signed gap only. The sample size rides the axis tick below the
    # bar instead — at full label size "· n=NN" made the two-line label wider
    # than the bar slot, so neighbouring labels collided and the last one clipped.
    text=[
        f"{v:.0f}%<br><span style='font-size:10px'>{g:+.1f}pp</span>"
        for v, g in zip(cal_df["actual_win_rate"] * 100, cal_df["gap_pp"])
    ],
    textposition="outside",
    textfont=dict(color="#c8c8cf", size=11, family="JetBrains Mono"),
    # Plotly's default constraintext="both" scales outside labels down to the bar
    # width, which at width=0.62 rendered this two-line label at ~60% of its
    # specified size. The label carries the colour-blind fallback, so it holds
    # its size and the bar keeps its width.
    constraintext="none",
    hovertemplate=(
        "Confidence: %{x}<br>"
        "Claimed: %{customdata[1]:.1f}%<br>"
        "Delivered: %{y:.1f}%<br>"
        "Gap: %{customdata[2]}<br>"
        "Fights: %{customdata[0]}<extra></extra>"
    ),
    # The signed gap is pre-formatted here rather than in the template —
    # hovertemplate's format spec drops the leading "+" sign flag.
    customdata=[
        [int(n), float(p), f"{g:+.1f}pp — {v}"]
        for n, p, g, v in zip(cal_df["count"], cal_df["avg_predicted"], cal_df["gap_pp"], bar_labels)
    ],
))

fig_conf.add_hline(y=50, line_dash="dash", line_color="#555")

# Every bar crosses y=50, so an in-plot label would sit on top of a bar —
# annotations paint above the bar layer, so it would erase the fill underneath.
# The label lives in the right margin instead, level with the line it names.
fig_conf.add_annotation(
    x=1, xref="paper", xanchor="left", xshift=8,
    y=50, yref="y", yanchor="middle",
    text="Coin flip", showarrow=False,
...
fig_conf.update_layout(
    ...
    font=dict(color="#f5f5f5", family="Exo 2"),
    xaxis=dict(
        title="Model Confidence Bucket", gridcolor="#1c1c20",
        tickmode="array",
        tickvals=list(cal_df["bucket"].astype(str)),
        ticktext=[
            f"{b}<br><span style='font-size:9px;color:#898781'>n={n}</span>"
            for b, n in zip(cal_df["bucket"].astype(str), cal_df["count"])
        ],
    ),
    yaxis=dict(title="Actual Accuracy %", gridcolor="#1c1c20", range=[30, 108]),
    margin=dict(l=50, r=76, t=20, b=50),  # right margin holds the "Coin flip" label
    height=350,
    showlegend=False,
)
